...
char message4[ ]= "MESSAGE FROM PROCESSOR4\n";
int main()
{
	int ret;
	mqd_t mqopen;
	...
	mqopen=mq_open("/posix4",O_CREAT|O_RDWR,0644,NULL); //create a posix message queue or open and existing queue
	perror("mq_open: ");

	ret=mq_send(mqopen,message4,strlen(message4)+1,101);  //send a message to message queue
	perror("msq_send: ");
